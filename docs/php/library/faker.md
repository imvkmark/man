...
每个生成器属性 (例如 name, address, lorem) 都被称谓 "formatters". 一个faker生成器有很多, 打包在 `Provider` 中。下面是默认区域中绑定的格式化程序的列表。
...
Faker提供三个特殊的提供者 providers, unique(), optional(), valid(), 在任何提供者之前调用。.
